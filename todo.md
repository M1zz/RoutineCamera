...
- [ ] **Production 스키마 배포**: `Feedback` 레코드 타입(+ recipientId/authorId/dateString/mealType Queryable), `RCGroup.visibility` 필드
  - Development 스키마에도 없으면 디버그 빌드로 한 번 사용하거나 콘솔에서 직접 생성 후 Deploy (CLOUDKIT_SETUP.md 참고)
- [x] 그룹 만들기/참여 시트: 키보드 내려가지 않던 문제 (탭·스크롤·완료 버튼으로 닫힘)
...
## 완료: 식후 사진 옵션 + 닉네임 노출 (빌드 검증 완료 ✅)
- [x] 설정 → 화면에 "식후 사진 남기기" 토글 (식단 모드 전용, 기본 ON)
  - 끄면: 카메라의 식전/식후 선택 숨김·항상 1장 저장, "다 드셨어요?" 알림 미예약 + 기존 예약 취소,
    빨간 "1" 뱃지 숨김, 상세 화면 식후 페이지 제거, 순간 피드에서 "기록 완료"로 표시
- [x] 친구 화면 상단에 "친구에게 보이는 이름" 카드 (탭 → 계정 설정에서 수정)
- [x] 계정 설정 시트에 이름 편집 행 추가, 설정 화면 닉네임 행도 같은 문구로 정리
- [x] 닉네임 CloudKit 저장을 1초 디바운스 (한 글자마다 업로드하던 것 수정) + 공백/빈 이름 방어
